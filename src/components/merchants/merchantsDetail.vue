    ...
    <div class="msg">
      <div class="msg-title">
        ...
        <div class="address" v-if="type==0">
          ...
          <div class="cur" v-if="projectDetail.project_address"><span><i>地</i>址：</span><b>{{projectDetail.project_address}}</b></div>
        </div>
        ...
        <div class="map" id="allmap1" @click="mapBtn()"></div>
      </div>
      ...
      <div class="xinxi" v-if="type===0">
        ...
        <ul>
          ...
          <li v-if="projectDetail.opened_date" class="cur"><span>开业时间：</span><b>{{projectDetail.opened_date}}</b></li>
  ...
  export default {
    ...
    methods:{
      ...
      mapFun(){// 百度地图API功能
        ...
        var point = new BMap.Point(this.projectDetail.lng,this.projectDetail.lat);
        // var point = new BMap.Point(39.911365,116.4573195952);
      ...
      back(){
        if(this.$route.query.type == 2 && this.$route.query.url){
          this.$router.push({path:this.$route.query.url,query:{type:2}})
      ...
      tuijianFun(item){//推荐详情
        window.location.href = "http://"+window.location.host+"/merchantsDetail?"+"projectId="+item.projectId+"&projectType="+item.projectType+"&houseType="+item.houseType;
        window.scrollTo('0','0');
        // this.$router.push({path:"/merchantsDetail",query:{projectId: item.projectId,projectType:this.projectType,houseType:item.houseType}})
      }
    }
  }
</script>
